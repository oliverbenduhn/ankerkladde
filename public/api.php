...
require dirname(__DIR__) . '/src/QuickAddHandler.php';
require dirname(__DIR__) . '/src/AddItemHandler.php';
...
function ensureUtf8(mixed $value): mixed
{
    if (is_string($value)) {
        if (!mb_check_encoding($value, 'UTF-8')) {
            $detectedEncoding = mb_detect_encoding($value, 'UTF-8, Windows-1252, ISO-8859-1', true) ?: 'Windows-1252';
...
try {
    ...
    switch ($action) {
        ...
        case 'today':
            ...
            // Auswahl liegt in fetchAgendaRows() (db.php), damit index.php beim
            // ersten Paint exakt dieselben Items rendert.
            $items = array_map(static function (array $row): array {
                $formatted = formatListItem($row);
                $formatted['agenda_group'] = $row['agenda_group'];
                return $formatted;
            }, fetchAgendaRows($db, $userId, $date, $today));
        ...
        case 'quick_add':
            handleQuickAdd($db, $userId, requireWriteRequest());

        case 'add':
            handleAddItem($db, $userId, requireWriteRequest());
